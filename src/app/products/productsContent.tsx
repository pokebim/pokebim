          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <div className="mt-2 text-gray-400 text-xs flex flex-wrap items-center">
                ...
                <button 
                  onClick={async () => {
                    // Al recargar precios, forzar actualización de todos los que tienen URL
                    if (window.confirm('¿Deseas actualizar todos los precios desde Cardmarket? Esto sobreescribirá los precios existentes.')) {
                      setLoadingPrices(true);
                      try {
                        const productsWithUrl = products.filter(p => p.cardmarketUrl && p.cardmarketUrl.trim() !== '');
                        if (productsWithUrl.length > 0) {
                          showNotification('Actualizando precios desde Cardmarket...', 'success');
                          await updateAllCardmarketPrices(productsWithUrl, true);
                        }
                        // Refrescar productos después de actualizar precios
                        await fetchProducts();
                  ...
                  disabled={loading || loadingPrices || isUpdatingPrices}
                >
                  {loadingPrices ? (
                    <>
